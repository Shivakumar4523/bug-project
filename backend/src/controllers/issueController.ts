...
export const issueController = {
  ...
  watch: (async (req, res) => res.json(await Issue.findByIdAndUpdate(req.params.id, { $addToSet: { watchers: req.user!.id } }, { new: true }))) as RequestHandler,
  unwatch: (async (req, res) => res.json(await Issue.findByIdAndUpdate(req.params.id, { $pull: { watchers: req.user!.id } }, { new: true }))) as RequestHandler,
  comments: (async (req, res) => res.json(await Comment.find({ issue: req.params.id }).populate("author", "name email").sort({ createdAt: 1 }))) as RequestHandler,
