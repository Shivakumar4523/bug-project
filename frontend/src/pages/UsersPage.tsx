...
import { Alert, Box, Button, Dialog, DialogActions, DialogContent, DialogTitle, IconButton, InputAdornment, MenuItem, Stack, Switch, Table, TableBody, TableCell, TableHead, TableRow, TextField, Typography } from "@mui/material";
...
function UserForm({ user, onCancel, onSubmit }: { user?: User | null; onCancel: () => void; onSubmit: (data: Partial<UserFormData>) => void }) {
  const { register, handleSubmit, reset } = useForm<UserFormData>({ defaultValues: valuesFromUser(user) });
...
export function UsersPage() {
  ...
  return (
    <>
      ...
      />
      <Table size="small"><TableHead><TableRow>{["Name", "Email", "Role", "Disabled", "Actions"].map((h) => <TableCell key={h}>{h}</TableCell>)}</TableRow></TableHead><TableBody>{filteredUsers.map((u) => {
